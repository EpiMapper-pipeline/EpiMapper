...
## Run Demos:
There are currently two demos avalible for using the EpiMapper Python package, one for CUT&Tag data and one for ATAC-seq data. Demo data can be found in the [demos](https://github.com/EpiMapper-pipeline/EpiMapper/tree/main/demos) folder and you can check out [demos documentation](https://epimapper-pipeline.github.io/EpiMapper/content/Demos.html) for more imforamtion. 

Ensure `The index files` for bowtie2 such as bowtie2_index_ecoli and bowtie2_index_hg38 shall be generated by yourself before running alignment. Or you can download them from the `in` section according to the above documentation. Also, please make sure to create the `output` folder before running the demo.
