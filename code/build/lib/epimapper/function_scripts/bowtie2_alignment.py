...
def bowtie2(fastq, spike_in, ref, projPath, files):
    ...
    for i in range(int(len(sample_names)/2)):
        ...
        cmd="bowtie2 --end-to-end --very-sensitive --no-mixed --no-discordant --phred33 -I 10 -X 700 -p 8 -x " + ref + "/"+ indexing+ \
            " --rg-id " + tmp_name + " --rg SM:" + tmp_name + " --rg LB:Library_ID"  + " --rg PU:" + tmp_name + "_Library_ID" + " --rg PL:Illumina" + \
            " -1 " + fastq +"/" +R1 +".fastq" +\
            " -2 " +fastq+"/"+R2+".fastq -S " + projPath +"/Epimapper/alignment/sam/"+tmp_name+".sam &> "+ projPath+"/Epimapper/alignment/bowtie2_summary/"+tmp_name+".txt" 
        #end jbw
