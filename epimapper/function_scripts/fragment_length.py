...
def smooth_fragment_length_distribute(frag_length_sample, sample_name, local_pct, se_method):
        ...
        ## calculate the confidence_interval based on different methods
...
def summary(fraglen, summary_tables, local_pct, se_method):
    ...
    Function input:
        ...
        * local_pct, float, define the percentage of local data points to consider when calculating the local standard error

        * se_method, int, define the method of calculating standard error
    ...
    Runs function:
        ...
        *plot_len_lineplot() or plot_len_lineplot_smooth() 
    ...
    if se_method == 0:
        print("Plot the original fragment length distribution without confidence intervals")
        plot_len_lineplot(frag_length_summary, summary_tables)

    else:
        #jbw 2024
        if se_method==1:
           print("Plot locally smoothed fragment length distribution with confidence intervals ", str(local_pct))
        elif se_method==2:
           print("Plot global smoothed fragment length distribution with confidence intervals ")
        else:
           print("Unknown smooth method for fragment length distribution ", str(se_method), " I Stop! ")
           exit(1)
